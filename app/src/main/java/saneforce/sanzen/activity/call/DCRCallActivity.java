    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        root.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            ...
            if (keypadHeight > screenHeight * 0.15) {
                findViewById(R.id.rl_btns).setVisibility(View.GONE); // keyboard open
            } else {
                findViewById(R.id.rl_btns).setVisibility(View.VISIBLE); // keyboard close
            }
        });
                    ...
                    try {
                        if (SharedPref.getEditCallOtpNeed(DCRCallActivity.this).equalsIgnoreCase("0") && isFromActivity.equalsIgnoreCase("edit_online") && SharedPref.getProductOtpNeed(DCRCallActivity.this).equalsIgnoreCase("0")) {
                            for (int i = 0; i < CheckProductListAdapter.saveCallProductListArrayList.size(); i++) {
                                ...
                                String productData = product.getCode() + "-" + product.getPromoted() + "-" + sampleQty + "-" + rxQty + "-" + rcpaQty;
...
//                            }
        ...
        /*    if (js.has("Product_Stockist")) {
                ...
                for (int m = 0; m < prdNameList.size(); m++) {
                    ...
                    try {
                        String productData = prdCodeList.get(m) + "-" + (promotedProducts.contains(prdNameList.get(m)) ? "0" : "1") + "-" + prdSamQtyList.get(m) + "-" + prdRxQtyList.get(m) + "-" + prdRcpaQtyList.get(m);
