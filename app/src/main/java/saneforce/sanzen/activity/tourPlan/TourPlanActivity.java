...
public class TourPlanActivity extends AppCompatActivity {
    ...
    public static ArrayList<OneBuildModelClass> dayWiseArrayPrevMonthOneBuild = new ArrayList<>();
    ...
    public static ModelClass.SessionList prepareSessionListForAdapter1
            (ArrayList<ModelClass.SessionList.SubClass> clusterArray, ArrayList<ModelClass.SessionList.SubClass> jcArray, ArrayList<ModelClass.SessionList.SubClass> drArray, ArrayList<ModelClass.SessionList.SubClass> chemistArray, ArrayList<ModelClass.SessionList.SubClass> stockArray, ArrayList<ModelClass.SessionList.SubClass> unListedDrArray, ArrayList<ModelClass.SessionList.SubClass> cipArray, ArrayList<ModelClass.SessionList.SubClass> hospArray, ModelClass.SessionList.WorkType
                    workType, ModelClass.SessionList.SubClass hq) {
     ...
     /*   Handler handler = new Handler();
        ...
        if (SharedPref.getOneBuild(context).equalsIgnoreCase("0")) {
          /*  TourPlanOfflineDataTable tourPlanOfflineDataTable = tourPlanOfflineDataDao.getTpDataOfMonthOrNew(TimeUtils.GetConvertedDate(TimeUtils.FORMAT_4, TimeUtils.FORMAT_23, String.valueOf(localDate)));
            ...
            if (Objects.equals(changeStatus, "0") || Objects.equals(changeStatus, "-1")){
                binding.tvSync.setText("Draft/Sync");
            }*/
                            ...
                            if (modelClass1.getWorkType().getName().isEmpty()) {
                                ...
                                if (modelClass1.getHeadquarters().getName().isEmpty() && SharedPref.getSfType(TourPlanActivity.this).equalsIgnoreCase("2") && !SharedPref.getOneBuild(TourPlanActivity.this).equalsIgnoreCase("0")) {
                                    ...
                                    if (FW_meetup_mandatory.equals("0")) {
                                        ...
                                        if (modelClass1.getDoctors().size() == 0 && modelClass1.getChemists().size() == 0 && modelClass1.getStockLists().size() == 0 && modelClass1.getUnlistedDoctors().size() == 0 && modelClass1.getCip().size() == 0 && modelClass1.getHospitals().size() == 0) {
                                            isEmpty = true;
                                            position = i;
...
//                                }else
                                ...
                                if ((SharedPref.getSfType(TourPlanActivity.this).equalsIgnoreCase("1") && modelClass.getListedDr().isEmpty() && modelClass.getChemist().isEmpty() && modelClass.getStockiest().isEmpty() && modelClass.getUnListedDr().isEmpty() && modelClass.getCip().isEmpty() && modelClass.getHospital().isEmpty())
                                        || (SharedPref.getSfType(TourPlanActivity.this).equalsIgnoreCase("2") && !SharedPref.getOneBuild(TourPlanActivity.this).equalsIgnoreCase("0") && modelClass.getListedDrs().isEmpty() && modelClass.getChemists().isEmpty() && modelClass.getStockiests().isEmpty() && modelClass.getUnListedDrs().isEmpty() && modelClass.getCips().isEmpty() && modelClass.getHospitals().isEmpty())) {
                                    isEmpty = true;
                                    position = i;
                                    commonUtilsMethods.showToastMessage(TourPlanActivity.this, getString(R.string.select_any) + " " + masters + " " + getString(R.string.in_session) + (i + 1));
                                    break;
...
//                            }else
    ...
    public ArrayList<OneBuildModelClass> prepareModelClassForMonthOneBuild
                    ...
                    if (!day.isEmpty()) {
                       ...
                       /*     if (isSTPBasedTP && !dayOfWeek.isEmpty() && stpOfflineDataDao.isDayAvailable(dayOfWeek)) {
                                ...
                                if (holidayDateArray.contains(day)) {
                                    ...
                                    ArrayList<OneBuildModelClass.SessionList> sessionLists = new ArrayList<>();
                                    sessionLists.add(sessionListOneBuild);
                                    oneBuildModelClass = new OneBuildModelClass("", day, date, dayName, month, year, false, sessionLists);
                                } else if (weeklyOffDays.contains(dayName)) {// add weekly off object when the day is declared as Weekly Off
...
//            saveTpLocal(modelClasses, day, monthYear, "0");
    ...
    private List<ModelClass.SessionList.SubClass> prepareList(String codes, String
            names) {
    ...
    private List<OneBuildModelClass.SessionList.SubClass> prepareListOneBuild
            (String codes, String names) {
        SharedPref.getOneBuild(TourPlanActivity.this).equalsIgnoreCase("0");
...
//                }
    ...
    public void getDraftSaveOneBuild(String
                                             isClickedName, ArrayList<OneBuildModelClass> arrayList, String isFrom,
        ...
        NetworkStatusTask networkStatusTask = new NetworkStatusTask(this, status -> {
            if (status) {
                try {
                    ...
                    tourPlan.addProperty("Month", Integer.parseInt(oneBuildModelClassTp.getMonth()));
                    tourPlan.addProperty("Year", Integer.parseInt(oneBuildModelClassTp.getYear()));
...
//                        ArrayList<String> dummy = new ArrayList<>();
                                                ...
                                                if (jsonArray.length() >= 0) {
                                                    arrayList = new Gson().fromJson(String.valueOf(jsonArray), type);
                                                    sendTpForApprovalOneBuild(jbonj, arrayList, localDate.toString(), oneBuildModelClassTp.getMonth(), statusOffline, isClickedName);
//                                                for (OneBuildModelClass oneBuildModelClass : arrayList) {
...
//                                                        binding.progressBar.setVisibility(View.VISIBLE);
//                                                        if (monthInAdapterFlag == -1) {
//                                                            prepareObjectToSendForApprovalOneBuild(TimeUtils.GetConvertedDate(TimeUtils.FORMAT_17, TimeUtils.FORMAT_23, oneBuildModelClass.getDate()), oneBuildModelClass.getDayNo(), arrayList, true, "prev");
//                                                            Log.d("TP_sendAppr", "onCreate: "+oneBuildModelClass.getDayNo());
//                                                        } else if (monthInAdapterFlag == 0) {
//                                                            prepareObjectToSendForApprovalOneBuild(TimeUtils.GetConvertedDate(TimeUtils.FORMAT_17, TimeUtils.FORMAT_23, oneBuildModelClass.getDate()), oneBuildModelClass.getDayNo(), arrayList, true, "current");
//                                                        } else if (monthInAdapterFlag == 1) {
//                                                            prepareObjectToSendForApprovalOneBuild(TimeUtils.GetConvertedDate(TimeUtils.FORMAT_17, TimeUtils.FORMAT_23, oneBuildModelClass.getDate()), oneBuildModelClass.getDayNo(), arrayList, true, "next");
//                                                        }
//                                                    }
//                                                }
                                                }
    ...
    private void SaveTourPlanWholeMonth(JSONObject jsonObject1, String
            isClickedName) {
        try {
            localDate = LocalDate.now();
...
//                   saveTpLocalOneBuild(dayWiseArrayNextMonthOneBuild, day, monthName, status);
    ...
    private void SaveTpLocalFullOneBuild(ReceiveModel
                                                 receiveModel, ArrayList<OneBuildModelClass> oneBuildModelClasses, ArrayList<OneBuildModelClass> dayWiseSaveTp, String
    ...
    private ArrayList<OneBuildModelClass.SessionList.SubClass> addExtraDataOneBuild
...
/*    public void sendWholeMonthStatusOneBuild(LocalDate localDate1, String isClickedName) {
    ...
    public static String textBuilder_OB
            (List<OneBuildModelClass.SessionList.SubClass> sessionLists,
             boolean codeOrName) {
...
//                        JSONObject jsonObject1 = new JSONObject(response.body().toString());
    ...
    public void saveTpLocalOneBuild
            (ArrayList<OneBuildModelClass> arrayList, String date, String month, String
                    status) {
        SharedPref.getOneBuild(TourPlanActivity.this).equalsIgnoreCase("0");
        for (OneBuildModelClass oneBuildModelClass : arrayList) {
