...
public class VisitFilter {
    ...
    public static class MonthlyStats {
        ...
        public List<String> visitedStockiest = new ArrayList<>();
        public List<String> visitedUnlisted = new ArrayList<>();
                ...
                if (!rejectedDates.contains(callDateStr)) {
                    ...
                    if (!monthKey.isEmpty()) {
                        ...
                        if(custType.equalsIgnoreCase("1")){
                            stats.visitedDoctors.add(custType);
                            int count = stats.customerVisitCounts.getOrDefault(custType, 1);
                            stats.customerVisitCounts.put(custType, count + 1);
                        }

                        else if(custType.equalsIgnoreCase("2")){
                            stats.visitedChemists.add(custType);
                            int count = stats.customerVisitCounts.getOrDefault(custType, 2);
                            stats.customerVisitCounts.put(custType, count + 1);
                        }
                        else if(custType.equalsIgnoreCase("3")){
                            stats.visitedStockiest.add(custType);
                            int count = stats.customerVisitCounts.getOrDefault(custType, 3);
                            stats.customerVisitCounts.put(custType, count + 1);
                        }
                        else if(custType.equalsIgnoreCase("4")){
                            stats.visitedUnlisted.add(custType);
                            int count = stats.customerVisitCounts.getOrDefault(custType, 4);
                            stats.customerVisitCounts.put(custType, count + 1);
                        }



                        if (custType.equalsIgnoreCase("1") && !custCode.isEmpty()) {
                        ...
                        }else if(custType.equalsIgnoreCase("2") && !custCode.isEmpty()){
                            stats.uniqueChemists.add(custCode);
                            int count = stats.customerVisitCounts.getOrDefault(custCode, 0);
                            stats.customerVisitCounts.put(custCode, count + 1);
                        }else if(custType.equalsIgnoreCase("3") && !custCode.isEmpty()){
                            stats.uniqueStockiest.add(custCode);
                            int count = stats.customerVisitCounts.getOrDefault(custCode, 0);
                            stats.customerVisitCounts.put(custCode, count + 1);
                        }else if(custType.equalsIgnoreCase("4") && !custCode.isEmpty()){
