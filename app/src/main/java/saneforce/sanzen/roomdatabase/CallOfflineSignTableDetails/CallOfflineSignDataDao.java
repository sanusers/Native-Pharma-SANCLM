...
public interface CallOfflineSignDataDao {
    ...
    @Query("DELETE FROM `CALL_OFFLINE_SIGN_TABLE` WHERE `CALL_CUS_CODE_SIGN` = :cusCode AND `CALL_CUS_NAME_SIGN` = :cusName AND `CALL_DATE_SIGN` = :date")
    void deleteOfflineSign(String cusCode, String cusName, String date);

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    void saveOfflineSignData(CallOfflineSignDataTable callOfflineSignDataTable);

    @Query("SELECT * FROM `CALL_OFFLINE_SIGN_TABLE` WHERE `CALL_IMAGE_NAME_SIGN` = :imageName")
    ...
    @Query("SELECT EXISTS(SELECT 1 FROM `CALL_OFFLINE_SIGN_TABLE` WHERE `CALL_SYNC_STATUS_SIGN` = 0 AND `CALL_DATE_SIGN` = :date)")
    boolean isSignDataAvailable(String date);
    ...
    @Query("UPDATE `CALL_OFFLINE_SIGN_TABLE` SET `CALL_STATUS_SIGN` = :status, `CALL_SYNC_STATUS_SIGN` = :checkSynced WHERE `ID` = :id")
