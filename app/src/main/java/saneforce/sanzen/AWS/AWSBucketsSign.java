...
public class AWSBucketsSign {
    ...
    private class AWSbucketsDownloadSign extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... arg0) {
            try {
                TransferObserver downloadObserver = transferUtility.download("san-edet",SharedPref.getDivisionCode(context).replace(",","/")+"Signature"+"/"+ filestored_name+filename, file);
    ...
    private class AWSbucketsclassSign extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... arg0) {
            try {
                TransferObserver image_upload = transferUtility.upload("san-edet",SharedPref.getDivisionCode(context).replace(",","/")+ "Signature"+"/"+filestored_name+filename, file);
