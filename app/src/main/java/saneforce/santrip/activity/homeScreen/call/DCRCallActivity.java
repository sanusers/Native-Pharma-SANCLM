...
import static saneforce.santrip.activity.homeScreen.call.fragments.RCPAFragment.ChemistSelectedList;
import static saneforce.santrip.activity.homeScreen.call.fragments.RCPAFragment.ProductSelectedList;
...
import saneforce.santrip.activity.homeScreen.call.pojo.rcpa.RCPAAddedProdList;
...
public class DCRCallActivity extends AppCompatActivity {
            ...
            case "1":
                ...
                for (int i = 0; i < ProductSelectedList.size(); i++) {
                    ...
                    for (int j = 0; j < rcpa_comp_list.size(); j++) {
                        if (ProductSelectedList.get(i).getChe_codes().equalsIgnoreCase(rcpa_comp_list.get(j).getChem_Code()) && ProductSelectedList.get(i).getPrd_code().equalsIgnoreCase(rcpa_comp_list.get(j).getPrd_code())) {
    ...
    private void jsonExtractOnline(String jsonArrayOnline) {
        ...
        String prdPromoted = "", ChemName = "", CheCode = "";
        JSONArray remArray = new JSONArray();
        JSONObject remObj;
        int CompFullValues = 0;
        ...
        try {
            ...
            if (!json.getString("DigitalHead").equalsIgnoreCase("[]")) {
                ...
                for (int i = 0; i < jsonPrdSlides.length(); i++) {
                    ...
                    if (jsSlidesPrds.getString("GroupID").equalsIgnoreCase("1")) {
                        ...
                        DetailedFragment.callDetailingLists.add(new CallDetailingList(ProductName, ProductCode, "", "", "", StartTime.trim() + " " + EndTime.trim(), StartTime.trim(), Integer.parseInt(Rating), PrdFeedBack, Date));
                    }
            ...
            if (js.has("promoted_product")) {
                if (!js.getString("promoted_product").isEmpty()) {
                    prdPromoted = js.getString("promoted_product").replaceAll("#", ",");
                }
            }
        ...
        /*    if (js.has("Product_Stockist")) {
            ...
            if (funStringValidation(js.getString("Activity_Remarks")))
                editRemarks = js.getString("Activity_Remarks");

            //POB
            if (js.has("POB")) {
                editPob = js.getString("POB");
            }
            ...
            if (js.has("Drcallfeedbackcode") && js.has("Call_Fdback")) {
                ...
                editFeedback = js.getString("Call_Fdback");
            }
/*
            Log.v("jsonExtractOnline", "product--size--" + prds.size());
            Log.v("jsonExtractOnline", "product-sam-size--" + prdSamQty.size());
            Log.v("jsonExtractOnline", "product-rx-size--" + prdRXQty.size());
            Log.v("jsonExtractOnline", "product-promoted--" + prdPromoted);*/
            ...
            for (int m = 0; m < prdNameList.size(); m++) {
                for (int j = 0; j < ProductFragment.checkedPrdList.size(); j++) {
                    ...
                    if (PrdList.getCode().equalsIgnoreCase(prdCodeList.get(m))) {
                        ...
                        if (prdPromoted.contains(prdCodeList.get(m))) {
          ...
          /*  Log.v("jsonExtractOnline", "inputs--size--" + input.size());
            ...
            if (!json.getString("RCPAHead").equalsIgnoreCase("[]")) {
                ...
                for (int i = 0; i < jsonArrayRcpa.length(); i++) {
                    ...
                    for (int j = 0; j < jsonArrayComp.length(); j++) {
                        ...
                        CompFullValues = CompFullValues + Integer.parseInt(jsonComp.getString("CPValue"));
                        rcpa_comp_list.add(new RCPAAddedCompList(ChemName, CheCode, jsonRcpa.getString("OPName"), jsonRcpa.getString("OPCode"), jsonComp.getString("CompName"), jsonComp.getString("CompCode"), jsonComp.getString("CompPName"), jsonComp.getString("CompPCode"), jsonComp.getString("CPQty"), jsonComp.getString("CPRate"), jsonComp.getString("CPValue"), jsonComp.getString("CPRemarks"), String.valueOf(CompFullValues)));
                    }

                    ProductSelectedList.add(new RCPAAddedProdList(ChemName, CheCode, jsonRcpa.getString("OPName"), jsonRcpa.getString("OPCode"), jsonRcpa.getString("OPQty"), jsonRcpa.getString("OPRate"), jsonRcpa.getString("OPValue"), String.valueOf(CompFullValues)));
                    double getTotalValue = 0.0;
                    ArrayList<Double> double_data = new ArrayList<>();
                    if (ProductSelectedList.size() > 0) {
                        for (int m = 0; m < ProductSelectedList.size(); m++) {
                            if (ProductSelectedList.get(m).getChe_codes().equalsIgnoreCase(CheCode)) {
                                double_data.add(Double.parseDouble(ProductSelectedList.get(m).getTotalPrdValue()));
                            }
                        }
                    } else {
                        ChemistSelectedList.add(new CustList(ChemName, CheCode, String.valueOf(CompFullValues), ""));
                    }
                    ...
                    ChemistSelectedList.add(new CustList(ChemName, CheCode, String.valueOf(valueRounded), ""));

                    int count = ChemistSelectedList.size();
                    for (int m = 0; m < count; m++) {
                        for (int j = m + 1; j < count; j++) {
                            if (ChemistSelectedList.get(m).getCode().equalsIgnoreCase(ChemistSelectedList.get(j).getCode())) {
                                String value = ChemistSelectedList.get(j).getTotalRcpa();
                                ChemistSelectedList.remove(j--);
                                ChemistSelectedList.set(m, new CustList(ChemName, CheCode, value, ""));
                                count--;
                            }
                        }
                    }
            ...
            Log.v("jsonExtractOnline", "----" + ProductSelectedList.size() + "----" + rcpa_comp_list.size());
        } catch (Exception e) {
           ...
           /* if (json.has("EventCapture")) {
                ...
                if (js.getString("Group").equalsIgnoreCase("0")) {
                    for (int j = 0; j < ProductFragment.checkedPrdList.size(); j++) {
                        ...
                        if (js.getString("Code").equalsIgnoreCase(PrdList.getCode())) {
                            ...
                            CheckProductListAdapter.saveCallProductListArrayList.add(new SaveCallProductList(js.getString("Name"), js.getString("Code"), js.getString("category"), PrdList.getStock_balance(), String.valueOf(lastStock), samQty, js.getString("RxQty"), "", js.getString("Promoted"), true));
                            PrdList.setCheckedItem(true);
                            break;
                        }
                    }
                    ...
                    Log.v("slide_length_prinnt", String.valueOf(jsonArraySlide.length()));
                    String SlideType = "", SlideRemarks = "", SlideName = "", SlidePath = "", SlideRating = "0", SlideScr;
                    for (int j = 0; j < jsonArraySlide.length(); j++) {
                        ...
                        arrayStore.add(new StoreImageTypeUrl("", SlideName, SlideType, "", "0", "", remArray.toString(), js.getString("Name"), js.getString("Code"), false));
                    }
                }
            }
            ...
            if (jsonArrayEc.length() > 0) {
                for (int j = 0; j < jsonArrayEc.length(); j++) {
                    ...
                    callCaptureImageLists.add(new CallCaptureImageList(jsEC.getString("EventImageTitle"), jsEC.getString("EventImageDescription"), null, jsEC.getString("Eventfilepath"), jsEC.getString("EventImageName"), false));
                }
            }
        ...
        try {
          ...
          /*  //Additional Call
            ...
            for (int i = 0; i < ProductSelectedList.size(); i++) {
                ...
                json_che.put("Name", ProductSelectedList.get(i).getChem_names());
                json_che.put("Code", ProductSelectedList.get(i).getChe_codes());
                jsonArrayChemist.put(json_che);
                json_object.put("Chemists", jsonArrayChemist);

                json_object.put("OPCode", ProductSelectedList.get(i).getPrd_code());
                json_object.put("OPName", ProductSelectedList.get(i).getPrd_name());
                json_object.put("OPQty", ProductSelectedList.get(i).getQty());
                json_object.put("OPRate", ProductSelectedList.get(i).getRate());
                json_object.put("OPValue", ProductSelectedList.get(i).getValue());
                json_object.put("OPTotal", ProductSelectedList.get(i).getTotalPrdValue());
            ...
            if (isFromActivity.equalsIgnoreCase("new")) {
                ...
                jsonSaveDcr.put("address", CommonUtilsMethods.gettingAddress(this, Double.parseDouble(latEdit), Double.parseDouble(lngEdit), false));
            }
    ...
    private void AddRCPAData() {
        ChemistSelectedList = new ArrayList<>();
